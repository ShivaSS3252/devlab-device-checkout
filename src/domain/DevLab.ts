...
export class DevLab {
  ...
  addDevice(device: Device): DevLab {
    const existingDeviceIndex = this.devices.findIndex(d => d.name === device.name);
    ...
    return new DevLab([...this.devices, device], this.users);
  }

  returnToInventory(deviceName: string): DevLab {
    const existingDeviceIndex = this.devices.findIndex(d => d.name === deviceName);
    if (existingDeviceIndex !== -1) {
      const existingDevice = this.devices[existingDeviceIndex];
      const updatedDevice = existingDevice.incrementCopies();
      const updatedDevices = [...this.devices];
    ...
    return new DevLab([...this.devices, returnedDevice], this.users);
  }
  ...
  removeDevice(deviceName: string): DevLab {
    ...
    const updatedDevices = this.devices.filter(d => d.name !== deviceName);
    return new DevLab(updatedDevices, this.users);
  }

  addUser(user: User): DevLab {
    return new DevLab(this.devices, [...this.users, user]);
  }
  ...
  updateUser(user: User): DevLab {
    ...
    return new DevLab(this.devices, updatedUsers);
  }
