...
export function Toast({ toast, onClose }: ToastProps) {
  ...
  const getToastStyles = () => {
    const baseStyles = "flex items-start p-4 mb-4 text-sm border rounded-lg transition-all duration-300 transform"
  ...
  return (
    <div className={`${getToastStyles()} ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}>
      ...
      <button
        onClick={handleClose}
        className="ml-4 flex-shrink-0 text-current opacity-60 hover:opacity-100 focus:outline-none focus:opacity-100"
